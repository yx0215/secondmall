...
import com.yx.xx.transaction.service.GoodsService;
...
//        example.createCriteria().andEqualTo("sellerId",sellerId);
    ...
    @Override
    public PageInfo<Goods> page1(int draw, int start, int length, Goods goods,Long sellerId) {
        Map<String,Object> params = new HashMap<>();
        params.put("start",start);
        params.put("length",length);
        params.put("goods",goods);
        ...
        pageInfo.setRecordsTotal(count);
        pageInfo.setRecordsFiltered(count);
        pageInfo.setData(goodsMapper.goodsPageBySellerId(params));
...
//        example.createCriteria().andEqualTo("sellerId",id).andEqualTo("sellStatus",0);
        List<Goods> goods = goodsMapper.selectList(Wrappers.<Goods>lambdaQuery().eq(Goods::getSellerId,id).eq(Goods::getSellStatus,1));
        ...
        // 输出查询到的商品数据到控制台
        System.out.println("查询到的状态为1的商品数据：");
        for (Goods good : goods) {
